...
export function FriendsList({ dispatch }: FriendsListProps) {
  ...
  const { data: friendsList, isLoading: friendsListLoading, error: friendsListError } = useQuery<{
    userid: number,
    username: string,
    avatar_url: string,
    online: boolean,
  ...
  >({
    ...
    queryFn: async () => {
      try {
        const response = await axios.get(`/api/FriendsList/list/${userId}`);
        return response.data;
      } catch (error) {
                ...
                return (
                    <div className='FriendsListUser' key={index}>
                        <div className="Avatar">
                            <img className="Frame" src={friend?.avatar_url}></img>
                        </div>
                        <div className="User" onClick={() => handleOpenProfile(friend.userid, friend.username)}>
                            <div className="Frame">
