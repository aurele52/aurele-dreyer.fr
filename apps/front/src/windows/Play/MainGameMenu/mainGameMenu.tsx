...
export default function MainGameMenu(props: mainGameMenuProps) {
  ...
  const [gameEndInfo, setGameEndInfo] = useState<gameEndInfo>();
  ...
  function onEndMatch(data: gameEndInfo) {
    setPongDisplay(false);
    setGameEndInfo({ ...gameEndInfo, ...data });
  ...
  return (
    <>
      ...
      {endGameDisplay === true && (
        <GameEnd
          isVictorious={gameEndInfo.isVictorious}
          player1={gameEndInfo.player1}
          player2={gameEndInfo.player2}
        ></GameEnd>
      )}
