...
function loop(p: p5, socket: Socket) {
  ...
  drawBall(p);
  drawBoardMidline(p);
  if (xitemSize > 0 && yitemSize > 0) drawItem(p);
  scoreOne(p, oneScore);
  scoreTwo(p, twoScore);
}
...
function onSizeChange(p: p5) {
  width = document.getElementById("canva")?.getBoundingClientRect().width || -1;
  height =
    document.getElementById("canva")?.getBoundingClientRect().height || -1;
  if (width !== -1 && height !== -1) {
...
export default function Play(props: playProps) {
  ...
  const Sketch = (p: p5) => {
    p.setup = () => {
      width =
        document.getElementById("canva")?.getBoundingClientRect().width || -1;
      height =
        document.getElementById("canva")?.getBoundingClientRect().height || -1;
      if (width !== -1 && height !== -1) {
    ...
    p.draw = () => {
      if (
        document.getElementById("canva")?.getBoundingClientRect().width !=
          width ||
        document.getElementById("canva")?.getBoundingClientRect().height !=
          height
      ) {
        onSizeChange(p);
      }
